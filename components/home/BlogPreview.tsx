  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const { data } = await supabase
        .from("blog_posts")
        .select(
          "id, slug, title_tr, title_en, excerpt_tr, excerpt_en, cover_image, category, read_time, published_at"
        )
  ...
  return (
    <section className="py-12 sm:py-16 bg-gradient-to-br from-brand-green to-brand-green-dark text-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            <h2 className="text-2xl font-bold sm:text-3xl">{t("latestPosts")}</h2>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      {post.read_time && (
                        <span className="flex items-center gap-1 text-xs text-muted-foreground">
                          ...
                          {post.read_time} {t("readTimeShort")}
                        </span>
                      )}
